...
KMCE Cricket Championship 2026
  ...
  try {
    ...
    console.log('Team creation email sent to admins')
    return { success: true }
  } catch (error: any) {
...
export async function sendPaymentConfirmationEmail(teamData: TeamData & { paymentStatus: string }) {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #0f172a; color: #ffffff;">
      <h1 style="color: #22c55e; border-bottom: 2px solid #22c55e; padding-bottom: 10px;">🏆 Team Approved!</h1>
      
      <div style="background-color: #166534; padding: 20px; border-radius: 10px; margin: 20px 0; text-align: center;">
        <h2 style="color: #ffffff; margin: 0;">✅ SQUAD REGISTERED</h2>
        <p style="color: #bbf7d0; margin: 10px 0 0 0;">The team is now officially part of KPL 2026!</p>
      </div>

      <div style="background-color: #1e293b; padding: 20px; border-radius: 10px; margin: 20px 0;">
        <h2 style="color: #fbbf24; margin-top: 0;">Team Identity</h2>
        <p><strong>Team Name:</strong> ${teamData.teamName}</p>
        <p><strong>Team ID:</strong> ${teamData.teamId}</p>
        <p><strong>Department:</strong> ${teamData.department}</p>
        <p><strong>Status:</strong> ELIGIBLE & APPROVED</p>
      </div>
...
✅ SQUAD REGISTERED - The team is now officially part of KPL 2026!
...
KMCE Cricket Championship 2026
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"KMCE Cricket" <${process.env.EMAIL_USER}>`,
      to: NOTIFICATION_EMAILS.join(', '),
      subject: `Team Approved: ${teamData.teamName} (${teamData.department}) - KPL 2026`,
      text: textContent,
      html: htmlContent,
      priority: 'high',
      headers: {
    ...
    console.log('Approval confirmation email sent to admins')
    return { success: true }
  } catch (error: any) {
