...
export async function POST(request: NextRequest) {
  try {
    const { teamId, teamIds, action } = await request.json()

    if ((!teamId && !teamIds) || !action) {
      return NextResponse.json({ error: 'Missing teamId(s) or action' }, { status: 400 })
    }

    const idsToProcess = teamIds || [teamId]
    ...
    for (const id of idsToProcess) {
      ...
      if (action === 'approve_payment' || action === 'approve_team') {
        if (action === 'approve_payment') {
          const { error: pError } = await supabase.from('payments').update({ status: 'approved' }).eq('team_id', id)
          if (pError) updateError = pError
        }

        const { error: tError } = await supabase.from('teams').update({ approved: true }).eq('id', id)
        if (tError) updateError = tError

        if (!updateError) {
          try {
            const { data: team } = await supabase.from('teams').select('*').eq('id', id).single()
            const { data: players } = await supabase.from('team_players').select('*').eq('team_id', id)
        ...
        const { error: pError } = await supabase.from('payments').update({ status: 'rejected' }).eq('team_id', id)
        updateError = pError
      } else if (action === 'reject_team') {
        const { error: tError } = await supabase.from('teams').update({ approved: false, whatsapp_sent: false }).eq('id', id)
        updateError = tError
      } else if (action === 'delete_team') {
        const { data: players } = await supabase.from('team_players').select('hall_ticket').eq('team_id', id)
        ...
        await supabase.from('payments').delete().eq('team_id', id)
        const { error: tError } = await supabase.from('teams').delete().eq('id', id)
        updateError = tError
      } else if (action === 'request_repayment') {
        const { error: pError } = await supabase.from('payments').delete().eq('team_id', id)
        const { error: tError } = await supabase.from('teams').update({ approved: false }).eq('id', id)
        updateError = pError || tError
      }

      if (updateError) {
        console.error(`Error processing team ${id}:`, updateError)
        // We continue with other IDs even if one fails
      } else {
        // Log the action for this team
        try {
          const { data: { user } } = await supabase.auth.getUser()
          const adminId = user?.user_metadata?.hall_ticket || 'UNKNOWN_ADMIN'
    ...
    return NextResponse.json({
      success: true,
      message: 'Management actions processed successfully'
    })
